...
/*
	...
	Text Domain: christmas-greetings
...
function christmas_greetings_default(){
	...
	if($get_set==''){
		...
		@update_option('display_on_pages', wp_json_encode($display_on_pages));
		...
		$christmas_greetings_settings = wp_json_encode($christmas_greetings_settings);
		...
		$christmas_greetings_advance_settings = wp_json_encode($christmas_greetings_advance_settings);
...
function christmas_greetings_options(){
	...
	if ( ($admin_manager == 1) && ( isset( $_POST[ 'save' ] ) )){
		if ( isset( $_REQUEST['_wpnonce'] ) && wp_verify_nonce( $_REQUEST['_wpnonce'], 'Save_Changes' ) ) {
			extract($_POST);
			...
			if($lic_obj->is_technocrackers()){
				...
				@update_option('display_on_pages_new', wp_json_encode($display_on_pages_new));
				...
				$newyear_greetings_settings = wp_json_encode($newyear_greetings_settings);
				...
				$newyear_greetings_advance_settings = wp_json_encode($newyear_greetings_advance_settings);
	...
	if (isset($_REQUEST['activate_license'])){
        if($lic_obj->christmas_greetings_active($_REQUEST['sample_license_key'])){        	
        	...
        	echo '<div id="message" class="updated fade"><p><strong>'.esc_html( 'You license Activated successfuly...!!!' ).'</strong></p></div>';
        } else {
        	echo '<div id="message" class="updated fade" style="border-left-color:#a00;"><p><strong>'.esc_html( $lic_obj->err ).'</strong></p></div>';
        }
    }
	if (isset($_REQUEST['deactivate_license']) && $_REQUEST['deactivate_license'] == 'deactivate_license'){
        if($lic_obj->christmas_greetings_deactive()){
        	echo '<div id="message" class="updated fade"><p><strong>'.esc_html( 'You license Deactivated successfuly...!!!' ).'</strong></p></div>';
        } else {
        	echo '<div id="message" class="updated fade" style="border-left-color:#a00;"><p><strong>'.esc_html( $lic_obj->err ).'</strong></p></div>';
        }
    }
	echo '<div class="wrap"><h1>' . esc_html('Christmas Greetings') . '</h1>';

	if ($settings_saved == 'true'){ 
		echo '<div id="message" class="updated fade"><p><strong>'.esc_html( 'Options saved.' ).'</strong></p></div>';
	}
	if($settings_saved1 == 'false'){
		echo '<div id="message" class="updated fade" style="border-left-color:#a00;"><p><strong>'.esc_html( 'Options save Allow Only For Admin.' ).'</strong></p></div>';
	}
	...
	if ($new_sett != '') {
			...
			// Sanitize and validate each variable from genral_settings
			$radio_active = isset($new_sett_res->active) ? sanitize_text_field($new_sett_res->active) : '';
			$radio_snow = isset($new_sett_res->snow) ? sanitize_text_field($new_sett_res->snow) : '';
			$radio_font_snow = isset($new_sett_res->snow_font) ? sanitize_text_field($new_sett_res->snow_font) : '';
			$color_snow = isset($new_sett_res->snow_color) ? sanitize_text_field($new_sett_res->snow_color) : '';
			$radio_santa = isset($new_sett_res->santa) ? sanitize_text_field($new_sett_res->santa) : '';
			$tc_decoration_radio = isset($new_sett_res->decoraation_radio_tc) ? sanitize_text_field($new_sett_res->decoraation_radio_tc) : '';
			$tc_song_radio = isset($new_sett_res->song_radio_tc) ? sanitize_text_field($new_sett_res->song_radio_tc) : '';
			$radio_greetings = isset($new_sett_res->greetings) ? sanitize_text_field($new_sett_res->greetings) : '';
			$srt_date = isset($new_sett_res->start_date) ? sanitize_text_field($new_sett_res->start_date) : '';
			$en_date = isset($new_sett_res->end_date) ? sanitize_text_field($new_sett_res->end_date) : '';
			$gifteffect_res = isset($new_sett_res->gifteffect) ? sanitize_text_field($new_sett_res->gifteffect) : '';
			$box_type = isset($new_sett_res->box) ? sanitize_text_field($new_sett_res->box) : '';
			$tc_snow_radio = isset($new_sett_res->snow_radio_tc) ? sanitize_text_field($new_sett_res->snow_radio_tc) : '';
			$tc_santa_radio = isset($new_sett_res->santa_radio_tc) ? sanitize_text_field($new_sett_res->santa_radio_tc) : '';
			$tc_gift_radio = isset($new_sett_res->tc_gift_radio) ? sanitize_text_field($new_sett_res->tc_gift_radio) : '';

			// Sanitize and validate each variable from advance_settings
			...
			$message_res = isset($sett_res1->msg) ? wp_kses_post($sett_res1->msg) : ''; // Sanitize HTML content
			$coupon_code = isset($sett_res1->code) ? sanitize_text_field($sett_res1->code) : '';
			$product_res = isset($sett_res1->res_product_select) ? sanitize_text_field($sett_res1->res_product_select) : ''; // Sanitize as integer
			$radio_layout = isset($sett_res1->layout) ? sanitize_text_field($sett_res1->layout) : ''; // Sanitize as key

			// Continue using sanitized and validated variables in your code...
	}

	if(get_option('new_advance_settings')){
		$new_set_temp=get_option('new_advance_settings');
		$new_sett_res1=json_decode($new_set_temp);
		$new_product_res = isset($new_sett_res1->new_res_product_select) ? sanitize_text_field($new_sett_res1->new_res_product_select) : '';
	}
	$path =  plugin_dir_url( __FILE__ );	
	$produc_value=array();
	...
	if (class_exists('WooCommerce')) {
			...
			while ($the_query->have_posts()) : $the_query->the_post();
					if (!have_posts()) :
							$product_found = true;
							if (!empty($product_res) && is_array($product_res) && in_array(get_the_ID(), $product_res)) {
							...
							if (!empty($new_product_res) && is_array($new_product_res) && in_array(get_the_ID(), $new_product_res)) {
	...
	$product_json=wp_json_encode($produc_value);
	$new_product_json=wp_json_encode($new_produc_value);
	$pages_array=array();
				...
				<div class="tab_wrapper">
					...
					<div data-panes>
							...
							<div data-panes class="vertical-tab-content">
								<div class="techno_sub_tabs_christmas_con sitedecoration tab_christmas_general active">							
									<div class="select_option-wrap">
										<p class="btn-switch">
											<input type="hidden" name="siteurl" value="'.esc_attr( get_site_url() ).'">					
										  	<input type="radio" id="no" name="active" value="Inactive" checked id="Inactive"'; if ( isset ( $radio_active ) ) checked( esc_attr( $radio_active ), "Inactive" ); echo'class="btn-switch__radio btn-switch__radio_no" />		
										  	<input type="radio" id="yes" name="active" value="active"';if ( isset ( $radio_active ) ) checked( esc_attr( $radio_active ), "active" ); echo' class="btn-switch__radio btn-switch__radio_yes" />
									...
									<div class="tabcontent01 li">
										<div class="tc_bg1">
											<div class="tc_head">	
												<h3>Snow</h3>	
												<div class="switch">
													<input type="radio" class="switch-input_snow switch-input" name="snow_radio_tc" value="enable" id="snow_enable" checked '; if ( isset ( $tc_snow_radio ) ) checked( esc_attr( $tc_snow_radio ), "enable" ); echo'>
													<label for="snow_enable" class="switch-label switch-label-off">enable</label>
													<input type="radio" class="switch-input" name="snow_radio_tc" value="disable" id="snow_disable"'; if ( isset ( $tc_snow_radio ) ) checked( esc_attr( $tc_snow_radio ), "disable" ); echo'>
											...
											<div class="tc_body">
												<div id="snow_show-me" class="single-tab-content-wrap image-wrap hide-option ';if($tc_snow_radio=='enable'){ echo esc_attr( "show-option" );	} echo'">										
													<div class="tc_padding">
														<div> 
															<h5>snow 1</h5>
															<input type="radio" checked name="snow" value="dark" id="one"';if(isset($radio_snow)) checked( esc_attr( $radio_snow ),"dark"); echo'>
														...
														<div>
															<h5>snow 2</h5>
															<input type="radio" name="snow" value="light" id="two"'; if(isset($radio_snow)) checked( esc_attr( $radio_snow ),"light"); echo'>
														...
														<div>
															<h5>snow 3</h5>
															<input type="radio" name="snow" value="three" id="three"'; if(isset($radio_snow)) checked( esc_attr( $radio_snow ),"three"); echo'>
															<label for="three">
																<div id="three_img" class="role unselect ';if ( $radio_snow=="three" ){echo 'selected';} echo'" for="three">
																	<input style="display:none" checked class="radio_snow_font" type="radio" name="snow_font" id="snow_font1" value="❄" '; if(isset($radio_font_snow)) checked( esc_attr( $radio_font_snow ), "❄" ); echo'><label id="snow_lable1" class="snow_lable '; if($radio_font_snow=="❄"){ echo'selected_snow'; } echo ' " for="snow_font1"style=" margin-left:3px; margin-top:3px;" ><img draggable="false" style="width: 26px !important;height: 28px !important;" role="img" class="emoji" alt="❄" src="https://s.w.org/images/core/emoji/12.0.0-1/svg/2744.svg"></label>
																	<input style="display:none"  class="radio_snow_font" type="radio" name="snow_font" id="snow_font2" value="❅" '; if(isset($radio_font_snow)) checked( esc_attr( $radio_font_snow ), "❅" ); echo'><label id="snow_lable2" class="snow_lable '; if($radio_font_snow=="❅"){ echo'selected_snow'; } echo ' " for="snow_font2">❅</label>
																	<input style="display:none" class="radio_snow_font" type="radio" name="snow_font" id="snow_font3" value="❆" '; if(isset($radio_font_snow)) checked( esc_attr( $radio_font_snow ), "❆" ); echo'><label id="snow_lable3" class="snow_lable '; if($radio_font_snow=="❆"){ echo'selected_snow'; } echo ' " for="snow_font3" >❆</label>
																	<input type="text" value="'.esc_attr( $color_snow ).'" name="snow_color" class="jscolor">
																</div>
															</label>
														</div>	
													</div>
										...
										<div class="tabcontent03">
											<div class="tc_bg1">
												<div class="tc_head">	
													<h3> santa</h3>
													<div class="switch">
														<input type="radio" class="switch-input_santa switch-input" name="santa_radio_tc" value="enable_santa" id="enable_santa" checked '; if( isset ( $tc_santa_radio ) ) checked( esc_attr( $tc_santa_radio ), "enable_santa" ); echo'>
														<label for="enable_santa" class="switch-label switch-label-off">enable</label>
														<input type="radio" class="switch-input" name="santa_radio_tc" value="disable_santa" id="disable_santa"'; if( isset ( $tc_santa_radio ) ) checked( esc_attr( $tc_santa_radio ), "disable_santa" ); echo'>
														<label for="disable_santa" class="switch-label switch-label-on">disable</label>
														<span class="switch-selection"></span>
													</div>		
													<div class="clr"></div>
												</div>						
												<div id="santa_show-me"class="single-tab-content-wrap image-wrap hide-option ';if($tc_santa_radio=='enable_santa'){ echo esc_attr( "show-option" );	} echo'">
													<div class="tc_padding">
														<div> 
															<h5>santa floating</h5>
															<input type="radio" name="santa" checked value="floating" id="one2"'; if(isset($radio_santa)) checked( esc_attr( $radio_santa ),"floating"); echo'>
														...
														<div>
															<h5>santa moving</h5>
															<input type="radio" name="santa" value="moving" id="two2"'; if(isset($radio_santa)) checked( esc_attr( $radio_santa ), "moving"); echo'>
										...
										<div class="giftbox-position">
											<div class="tc_bg">	
												<div class="tc_head">	
													<h5>Giftbox</h5>
													<div class="switch">
														<input type="radio" class="switch-input_snow switch-input" name="gift_radio_tc" value="gift_enable" id="gift_enable" checked '; if(isset($tc_gift_radio)) checked( esc_attr( $tc_gift_radio ),"gift_enable"); echo'>
														<label for="gift_enable" class="switch-label switch-label-off">enable</label>
														<input type="radio" class="switch-input" name="gift_radio_tc" value="gift_disable" id="gift_disable"'; if (isset($tc_gift_radio)) checked( esc_attr( $tc_gift_radio ),"gift_disable"); echo'>
												...
												<div class="select-type">
													<span class="gift_box_tc hide-gift ';if($tc_gift_radio=='gift_enable'){ echo esc_attr( "show-gift" ); } echo'">
														<h5>select giftbox position</h5>
														<div class="select-type-left">
															<input  checked type="radio" name="box" value="left_align" id="boxleft"'; if(isset($box_type)) checked( esc_attr( $box_type ), "left_align" ); echo'>
															<label class="" for="boxleft">left</label>
														</div>						 
														<div class="select-type-center">
															<input  type="radio" name="box" value="center_align" id="boxcenter"'; if(isset($box_type)) checked( esc_attr( $box_type ), "center_align" ); echo'>
															<label  for="boxcenter">center</label>
														</div>
														<div class="select-type-right">
															<input type="radio" name="box" value="right_align" id="boxright" '; if(isset($box_type)) checked( esc_attr( $box_type ), "right_align" ); echo'>
															<label class="" for="boxright">right</label>
														</div>
													</span>
													<span class="gift_box_tc_msg hide-gift ';if($tc_gift_radio=='gift_disable'){ echo esc_attr( "show-gift" ); } echo'">
														<div class="select-type-left">
										...
										<div class="tabcontent03">
											<div class="tc_bg_aa">
												<div class="tc_head">	
													...
													<div class="switch">
														<input type="radio" class="switch-input_santa switch-input" name="decoraation_radio_tc" value="enable_decoration" id="enable_decoration" checked '; if ( isset ( $tc_decoration_radio ) ) checked( esc_attr( $tc_decoration_radio ), "enable_decoration" ); echo'>
														<label for="enable_decoration" class="switch-label switch-label-off">enable</label>
														<input type="radio" class="switch-input" name="decoraation_radio_tc" value="disable_decoration" id="disable_decoration"'; if ( isset ( $tc_decoration_radio ) ) checked( esc_attr( $tc_decoration_radio ), "disable_decoration" ); echo'>
												...
												<div class="tc_body">			
													<div id="decoration_img_show" class="single-tab-content-wrap image-wrap hide-option ';if($tc_decoration_radio=='enable_decoration'){ echo esc_attr( "show-option" );	} echo'">										
														<div class="tc_padding">
															<div> 														
																<label class="new_role" for=""><img id="" style="width:100% !important;" class="unselect" src="'.esc_attr( $path ).'img/decoration.png"></label>
															</div>
														</div>							
													</div>	
												</div>
									...
									<div class="tabcontent02 li">
												...
												<div class="tc_body">	
													<div class="select-type-date">
														<div class="">
															...
															<input type="text" value="'.esc_attr( $srt_date ).'" class="sdate" name="start_date" id="txtFrom"/>
														</div>						 
														<div class="">
															<label for="date_e" >End Date</label>
															<input type="text" class="edate" value="'.esc_attr( $en_date ).'" name="end_date" id="txtTo"/>
														</div>						
													</div>		
												</div>	
											</div>		
										...
										<div class="greetings-type">
											<div class="tc_bg gift_box_tc hide-gift ';if($tc_gift_radio=='gift_enable'){ echo esc_attr( "show-gift" ); } echo'">
												<div class="tc_head">
													...
													<div class="select-type">';
														...
														{ echo '
															<div class="select-type-message">
																<input class="check_msg" type="radio" name="greetings" value="message" checked id="greetings1"'; if ( isset ( $radio_greetings ) ) checked( esc_attr( $radio_greetings ), "message" ); echo'>
															...
															<div class="select-type-coupon">
																<input class="check_coupon" type="radio" name="greetings" value="coupon" id="greetings2"'; if ( isset ( $radio_greetings ) ) checked( esc_attr( $radio_greetings ), "coupon" ); echo'>
															...
															if(class_exists('WooCommerce')) 
															{
																...
																{
																	echo '<input class="check_product" type="radio" name="greetings" value="product_promo" id="greetings3" '; if ( isset ( $radio_greetings ) ) checked( esc_attr( $radio_greetings ), "product_promo" ); echo'>
												...
												<div class="tc_bg">
													<span class="gift_box_tc hide-gift ';if($tc_gift_radio=='gift_enable'){ echo esc_attr( "show-gift" ); } echo'">
														...
														<div class="tc_body">
															<div class="select-type">
																<div class="select-type-message">
																	<input checked type="radio" name="gifteffect" value="move" id="movingeffect"'; if ( isset ( $gifteffect_res ) ) checked( esc_attr( $gifteffect_res ), "move" ); echo'>
																	<label for="movingeffect"><img id="move_gift" class="unselect '; if ( $gifteffect_res=="move" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/zomm.gif"></label>
																</div>
																<div class="select-type-coupon">
																	<input  type="radio" name="gifteffect" value="bell" id="belleffect"'; if ( isset ( $gifteffect_res ) ) checked( $gifteffect_res, "bell" ); echo'>
																	<label  for="belleffect"><img id="bell_gift" class="unselect '; if ( $gifteffect_res=="bell" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/bell.gif"></label>
																</div>
																<div class="select-type-product">
																	...
																	<label for="sparkalffect"><img id="sparkalffect_gift" class="unselect ' ;if ( $gifteffect_res=="sparkalffect" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/sparker.gif"></label>
																</div>
															</div>
														</div>	
													</span>
											...
											<div class="tabcontent03">
												<div class="tc_bg_aa">
													<div class="tc_head">	
														...
														<div class="switch">
															<input type="radio" class="switch-input_santa switch-input" name="song_radio_tc" value="enable_song" id="enable_song" checked '; if ( isset ( $tc_song_radio ) ) checked( esc_attr( $tc_song_radio ), "enable_song" ); echo'>
															<label for="enable_song" class="switch-label switch-label-off">enable</label>
															<input type="radio" class="switch-input" name="song_radio_tc" value="disable_song" id="disable_song"'; if ( isset ( $tc_song_radio ) ) checked( esc_attr( $tc_song_radio ), "disable_song" ); echo'>
														...
														<div class="tc_body">
															<div class="select-type">																
																<div class="select-type-all">
																	<input type="radio" checked name="display_on" '.checked( esc_attr( get_option('christmas_display_on') ),'all',false).' value="all" id="display_on_all">
																	<label for="display_on_all">All Pages</label>
																</div>
																<div class="select-type-home">
																	<input type="radio" name="display_on" '.checked( esc_attr( get_option('christmas_display_on') ),'home',false).' value="home" id="display_on_home">
																	<label for="display_on_home">Home Page</label>
																</div>';if($lic_obj->is_technocrackers()){ echo '
																<div class="select-type-pages">
																	<input type="radio" name="display_on" '.checked( esc_attr( get_option('christmas_display_on') ),'pages',false).' value="pages" id="display_on_pages">
																	<label for="display_on_pages">Selected Pages</label>
																	<span style="display:'.(( esc_attr( get_option('christmas_display_on') )=='pages')?'inline':'none').';" class="select_pagis_aria">
										...
										<div>
												...
												<div class="tc_bg">
													...
													<div class="tc_body">
														<div class="part-wrap">
															...
															<div class="input-gp"><input type="radio" checked name="fontfamily" value="Great_Vibes" id="fontfamily1" '; if ( isset ( $radio_fontfamily ) ) checked( esc_attr( $radio_fontfamily ), "Great_Vibes" ); echo'>
																<label class="fontfamily1 " for="fontfamily1">Merry Christmas</label>
															</div>						 
															<div class="input-gp"><input type="radio" name="fontfamily" value="Lobster" id="fontfamily2" '; if ( isset ( $radio_fontfamily ) ) checked( esc_attr( $radio_fontfamily ), "Lobster" ); echo'>
																<label class="fontfamily2 " for="fontfamily2">Merry Christmas</label>
															</div>
															<div class="input-gp"><input type="radio" name="fontfamily" value="Dancing_Scrip" id="fontfamily3"'; if ( isset ( $radio_fontfamily ) ) checked( esc_attr( $radio_fontfamily ), "Dancing_Scrip" ); echo'>
														...
														<div class="part-wrap">
															...
															<div class="input-gp"><input type="radio" checked name="color" value="color1" id="color1"'; if ( isset ( $radio_color ) ) checked( esc_attr( $radio_color ), "color1" ); echo'>
																<label class="color1 " for="color1">Merry Christmas</label>
															</div>						 
															<div class="input-gp"><input type="radio" name="color" value="color2" id="color2"'; if ( isset ( $radio_color ) ) checked( esc_attr( $radio_color ), "color2" ); echo'>
																<label class="color2 " for="color2">Merry Christmas</label>
															</div>
															<div class="input-gp"><input type="radio" name="color" value="color3" id="color3"';if ( isset ( $radio_color ) ) checked( esc_attr( $radio_color ), "color3" ); echo'>
											...
											<div id="msg" class="message-setting ';if($radio_greetings=='message'){ echo "show-option";	} echo'">
												<div class="tc_bg">
													...
													<div class="tc_body">					
														<div class="part-wrap ">
															<textarea class="unselect" name="msg" maxlength="85" rows="4" cols="50" id="text_tc" placeholder="Enter message...">'. esc_html( $message_res ).'</textarea>
														</div>
													</div>
												</div>
											</div>
											<div id="coupon" class="coupon-code hide-option ';if($radio_greetings=='coupon'){ echo esc_attr( "show-option" );	} echo'">				
												<div class="tc_bg">	
													<div class="tc_head">
														<h3>COUPON CODE</h3>
														<div class="clr"></div>
													...
													<div class="tc_body">
														<div class="code-input">
															<input class="unselect" type="text" maxlength="10" value="'. esc_attr( $coupon_code ) .'"name="code" placeholder="Enter code">
														</div>	
													</div>			
												</div>
											</div>
											<div id="product" class="product-setting hide-option '.esc_attr( @$product_show ).'">
												<div class="tc_bg">
													<div class="tc_head">	
														<h3>Product</h3>
														<div class="clr"></div>
													...
													<div class="tc_body">				
														...
														<div class="part-wrap" style="margin-top:0"><h3>Layout</h3>
															<div class="layout-wrap">
																<div>								
																	<input type="radio" name="layout" checked value="layout1" id="layout1"'; if ( isset ( $radio_layout ) ) checked( esc_attr( $radio_layout ), "layout1" ); echo'>
																	<label class="role " for="layout1"><img id="layout1_img" class="unselect ' ;if ( $radio_layout=="layout1" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/product-img1.png"></label>
																</div>
																<div>									
																	<input type="radio" name="layout" value="layout2" id="layout2" '; if ( isset ( $radio_layout ) ) checked( esc_attr( $radio_layout ), "layout2" ); echo'>
																	<label class="role " for="layout2"><img id="layout2_img" class="unselect ' ;if ( $radio_layout=="layout2" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/product-img2.png"></label>
																</div>
																<div>									
																	<input type="radio" name="layout" value="layout3" id="layout3" '; if ( isset ( $radio_layout ) ) checked( esc_attr( $radio_layout ), "layout3" ); echo'>
																	<label class="role " for="layout3"><img id="layout3_img" class="unselect ' ;if ( $radio_layout=="layout3" ){echo esc_attr( 'selected' );} echo'" src="'.$path.'img/product-img3.png"></label>
																</div>
															</div>
														</div>
													</div>
						...
						if($lic_obj->is_technocrackers()){ 	
							...
							$adv_sett = get_option('new_advance_settings');
							$new_sett_res1 = json_decode($adv_sett);
								...
								<div class="sitedecoration tab_newyear_general techno_sub_tabs_new_con active">							
									<div class="new_select_option-wrap">
										<p class="new_btn-switch">				
										  	<input type="radio" id="new_no" name="new_active" value="new_Inactive" checked id="Inactive"'; if ( isset ( $new_radio_active ) ) checked( esc_attr( $new_radio_active ), "new_Inactive" ); echo'class="new_btn-switch__radio new_btn-switch__radio_no" />		
										  	<input type="radio" id="new_yes" name="new_active" value="new_active"';if ( isset ( $new_radio_active ) ) checked( esc_attr( $new_radio_active ), "new_active" ); echo' class="new_btn-switch__radio new_btn-switch__radio_yes" />
										...
										<div class="tc_bg1">
											<div class="tc_head">	
												...
												<div class="new_switch">
												  	<input type="radio" class="new_switch-input_snow new_switch-input" name="new_snow_radio_tc" value="new_enable" id="new_snow_enable" checked '; if ( isset ( $new_tc_snow_radio ) ) checked( esc_attr( $new_tc_snow_radio ), "enable" ); echo'>
												  	<label for="new_snow_enable" class="new_switch-label new_switch-label-off">enable</label>
												  	<input type="radio" class="new_switch-input" name="new_snow_radio_tc" value="disable" id="new_snow_disable"'; if ( isset ( $new_tc_snow_radio ) ) checked( esc_attr( $new_tc_snow_radio ), "disable" ); echo'>
											...
											<div class="tc_body">
												<div id="new_snow_show-me" class="single-tab-content-wrap image-wrap hide-option ';if($new_tc_snow_radio=='new_enable'){ echo esc_attr( "show-option" );	} echo'">										
													<div class="tc_padding">
														<div>
															<h5>firework 1</h5>
															<input type="radio" checked name="new_firework" value="firework1" id="firework_one"';if ( isset ( $radio_firework ) ) checked( esc_attr( $radio_firework ), "firework1" ); echo'>
															<label class="new_role" for="firework_one"><img id="firework1_id" class="unselect ';if($radio_firework=='firework1'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/dark-snow.png"></label>
														</div>
													<!--<div>
															<h5>firework 2</h5>
															<input type="radio" name="new_firework" value="firework2" id="firework_two"'; if ( isset ( $radio_firework ) ) checked( esc_attr( $radio_firework ), "firework2" ); echo'>
															<label class="new_role" for="firework_two"><img id="firework2_id" class="unselect ';if($radio_firework=='firework2'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/light-snow.png"></label>
														</div>-->
													</div>
												</div>	
											</div>	
										...
										<div class="tabcontent03">
											<div class="tc_bg1">
												<div class="tc_head">	
													<h3> santa</h3>
													<div class="new_switch">
													 	<input type="radio" class="new_switch-input_snow new_switch-input" name="new_santa_radio_tc" value="new_enable_santa" id="new_enable_santa" checked '; if ( isset ( $new_tc_santa_radio ) ) checked( esc_attr( $new_tc_santa_radio ), "new_enable_santa" ); echo'>
													  	<label for="new_enable_santa" class="new_switch-label new_switch-label-off">enable</label>
													  	<input type="radio" class="new_switch-input" name="new_santa_radio_tc" value="new_disable_santa" id="new_disable_santa"'; if ( isset ( $new_tc_santa_radio ) ) checked( esc_attr( $new_tc_santa_radio ), "new_disable_santa" ); echo'>
													  	<label for="new_disable_santa" class="new_switch-label new_switch-label-on">disable</label>
													  	<span class="new_switch-selection"></span>
													</div>		
													<div class="clr"></div>
												</div>						
												<div id="new_santa_show-me"class="single-tab-content-wrap image-wrap hide-option ';if($new_tc_santa_radio=='new_enable_santa'){ echo esc_attr( "show-option" );	} echo'">
													<div class="tc_padding">
														<div> 
															<h5>santa floating</h5>
															<input type="radio" name="new_santa" checked value="new_floating" id="new_one2"'; if ( isset ( $new_radio_santa ) ) checked( esc_attr( $new_radio_santa ), "new_floating" ); echo'>
															<label class="new_role2" for="new_one2"><img id="new_santa_one" class="unselect ';if($new_radio_santa=='new_floating'){ echo esc_attr( "selected" ); } echo' " src="'.$path.'img/santa-hello.gif"></label>
														</div>
														<div>
															<h5>santa moving</h5>
															<input type="radio" name="new_santa" value="new_moving" id="new_two2"'; if ( isset ( $new_radio_santa ) ) checked( esc_attr( $new_radio_santa ), "new_moving" ); echo'>
															<label class="new_role2" for="new_two2"><img id="new_santa_two" class="unselect ';if($new_radio_santa=='new_moving'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/santa.gif"></label>
														</div>
													</div>
												</div>
											</div>
										...
										<div class="giftbox-position">
											<div class="tc_bg">	
												<div class="tc_head">
													<h5>Giftbox</h5>
													<div class="switch">
														<input type="radio" class="switch-input_snow switch-input" name="new_gift_radio_tc" value="new_gift_enable" id="new_gift_enable" checked '; if ( isset ( $new_tc_gift_radio ) ) checked( esc_attr( $new_tc_gift_radio ), "new_gift_enable" ); echo'>
														<label for="new_gift_enable" class="switch-label switch-label-off">enable</label>
														<input type="radio" class="switch-input" name="new_gift_radio_tc" value="new_gift_disable" id="new_gift_disable"'; if ( isset ( $new_tc_gift_radio ) ) checked( esc_attr( $new_tc_gift_radio ), "new_gift_disable" ); echo'>
												...
												<div class="select-type">
													<span class="new_gift_box_tc hide-gift ';if($new_tc_gift_radio=='new_gift_enable'){ echo esc_attr( "show-gift" ); } echo'">
														<h5>select giftbox position</h5>
														<div class="new_select-type-left">
															<input  checked type="radio" name="new_box" value="left_align" id="new_boxleft"'; if ( isset ( $new_box_type ) ) checked( esc_attr( $new_box_type ), "left_align" ); echo'>
															<label for="new_boxleft">left</label>
														</div>
														<div class="new_select-type-center">
															<input  type="radio" name="new_box" value="center_align" id="new_boxcenter"'; if ( isset ( $new_box_type ) ) checked( esc_attr( $new_box_type ), "center_align" ); echo'>
															<label for="new_boxcenter">center</label>
														</div>
														<div class="new_select-type-right">
															<input type="radio" name="new_box" value="right_align" id="new_boxright" '; if ( isset ( $new_box_type ) ) checked( esc_attr( $new_box_type ), "right_align" ); echo'>
															<label for="new_boxright">right</label>
														</div>
													</span>
													<span class="new_gift_box_tc_msg hide-gift ';if($new_tc_gift_radio=='new_gift_disable'){ echo esc_attr( "show-gift" ); } echo'">
														<div class="select-type-left">
															<h5 style="color: rgb(255, 0, 0);">you can'."'".'t use additional greeting type because gift box is disable</h5>
														</div>	
													</span>
										...
										<div class="tabcontent03">
											<div class="tc_bg_aa">
												<div class="tc_head">	
													<h3> decoration image </h3>
													<div class="switch">
												  		<input type="radio" class="switch-input_santa switch-input" name="new_decoraation_radio_tc" value="new_enable_decoration" id="new_enable_decoration" checked '; if ( isset ( $new_tc_decoration_radio ) ) checked( esc_attr( $new_tc_decoration_radio ), "enable_decoration" ); echo'>
												  		<label for="new_enable_decoration" class="switch-label switch-label-off">enable</label>
												  		<input type="radio" class="switch-input" name="new_decoraation_radio_tc" value="new_disable_decoration" id="new_disable_decoration"'; if ( isset ( $new_tc_decoration_radio ) ) checked( esc_attr( $new_tc_decoration_radio ), "disable_decoration" ); echo'>
												...
												<div class="tc_body">			
													<div id="new_decoration_img_show" class="single-tab-content-wrap image-wrap hide-option ';if($new_tc_decoration_radio=='new_enable_decoration'){ echo esc_attr( "show-option" );	} echo'">										
														<div class="tc_padding">
															<div> 														
																<label class="new_role" for=""><img id="" style="width:100% !important;" class="unselect" src="'.esc_attr( $path ).'img/decoration.png"></label>
															</div>
									...
									<div class="tabcontent02 li">
												...
												<div class="tc_body">	
													<div class="select-type-date">
														<div class="">
															...
															<input type="text" value="'. esc_attr( $new_srt_date ) .'" class="new_sdate" name="new_start_date" id="new_txtFrom"/>
														</div>						 
														<div class="">
															<label for="new_date_e" >End Date</label>
															<input type="text" class="new_edate" value="'. esc_attr( $new_en_date ) .'" name="new_end_date" id="new_txtTo"/>
														</div>					
													</div>		
												</div>	
											</div>		
										...
										<div class="greetings-type">
											<div class="tc_bg new_gift_box_tc hide-gift ';if($new_tc_gift_radio=='new_gift_enable'){ echo esc_attr( "show-gift" ); } echo'">
												<div class="tc_head">
													<h5>select greetings type</h5>
												</div>
												<div class="tc_body">
													<div class="select-type">
														<div class="select-type-message">
														<input class="new_check_msg" type="radio" name="new_greetings" value="new_message" checked id="new_greetings1"'; if ( isset ( $new_radio_greetings ) ) checked( esc_attr( $new_radio_greetings ), "new_message" ); echo'>
														...
														<div class="select-type-coupon">
															<input class="new_check_coupon" type="radio" name="new_greetings" value="new_coupon" id="new_greetings2"'; if ( isset ( $new_radio_greetings ) ) checked( esc_attr( $new_radio_greetings ), "new_coupon" ); echo'>
															<label for="new_greetings2">COUPON CODE</label>
														</div>
														<div class="select-type-product">';
															if(class_exists('WooCommerce')){
																if ($product_found==true){ echo '
																	<input class="new_check_product" type="radio" name="new_greetings" value="new_product_promo" id="new_greetings3" '; if ( isset ( $new_radio_greetings ) ) checked( esc_attr( $new_radio_greetings ), "new_product_promo" ); echo'>
													...
													<div class="tc_body">
														<div class="select-type">
															<div class="new_select-type-message">
																<input checked type="radio" name="new_gifteffect" value="move" id="new_movingeffect"'; if ( isset ( $new_gifteffect_res ) ) checked( esc_attr( $new_gifteffect_res ), "move" ); echo'>
																<label for="new_movingeffect"><img id="new_move_gift" class="unselect ';if($new_gifteffect_res=='move'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/zomm.gif"></label>
															</div>						 
															<div class="new_select-type-coupon">
																<input  type="radio" name="new_gifteffect" value="bell" id="new_belleffect"'; if ( isset ( $new_gifteffect_res ) ) checked( esc_attr( $new_gifteffect_res ), "bell" ); echo'>
																<label for="new_belleffect"><img id="new_bell_gift" class="unselect ';if($new_gifteffect_res=='bell'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/bell.gif"></label>
															</div>
															<div class="new_select-type-product">
																<input type="radio" name="new_gifteffect" value="sparkalffect" id="new_sparkalffect" '; if ( isset ( $new_gifteffect_res ) ) checked( esc_attr( $new_gifteffect_res ), "sparkalffect" ); echo'>
																<label for="new_sparkalffect"><img id="new_sparkalffect_gift" class="unselect ';if($new_gifteffect_res=='sparkalffect'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/sparker.gif"></label>
															</div>
														</div>
													</div>
												</div>
											...
											<div class="tabcontent03">
												<div class="tc_bg_aa">
													<div class="tc_head">	
														<h3> NewYear Music</h3>
														<div class="switch">
															<input type="radio" class="switch-input_santa switch-input" name="new_song_radio_tc" value="enable_song" id="new_enable_song" checked '; if ( isset ( $new_tc_song_radio ) ) checked( esc_attr( $new_tc_song_radio ), "enable_song" ); echo'>
															<label for="new_enable_song" class="switch-label switch-label-off">enable</label>
															<input type="radio" class="switch-input" name="new_song_radio_tc" value="disable_song" id="new_disable_song"'; if ( isset ( $new_tc_song_radio ) ) checked( esc_attr( $new_tc_song_radio ), "disable_song" ); echo'>
												...
												<div class="tc_bg">
													<span class="gift_box_tc hide-gift show-gift">
														...
														<div class="tc_body">
															<div class="select-type">																
																<div class="select-type-all">
																	<input type="radio" checked name="new_display_on" '.checked(esc_attr( get_option('newyear_display_on')),'all',false).' value="all" id="new_display_on_all">
																	<label for="new_display_on_all">All Pages</label>
																</div>
																<div class="select-type-home">
																	<input type="radio" name="new_display_on" '.checked(esc_attr( get_option('newyear_display_on')),'home',false).' value="home" id="new_display_on_home">
																	<label for="new_display_on_home">Home Page</label>
																</div>																
																<div class="select-type-pages">
																	<input type="radio" name="new_display_on" '.checked(esc_attr( get_option('newyear_display_on')),'pages',false).' value="pages" id="new_display_on_pages">
																	<label for="new_display_on_pages">Selected Pages</label>
																	<span style="display:'.((esc_attr( get_option('newyear_display_on'))=='pages')?'inline':'none').';" class="new_select_pages_aria">
																		(<small>Leave empty for display on all pages</small>)
								...
								<div class="template teb_newyear_advance techno_sub_tabs_new_con">
									...
									<div class="template-tab-content">
										<div>
													...
													<div class="tc_body">
														<div class="part-wrap">
															...
															<div class="new_input-gp">
																<input type="radio" checked name="new_fontfamily" value="Great_Vibes" id="new_fontfamily1" '; if ( isset ( $new_radio_fontfamily ) ) checked( esc_attr( $new_radio_fontfamily ), "Great_Vibes" ); echo'>
																<label class="new_fontfamily1" for="new_fontfamily1">Happy New Year</label>
															</div>						 
															<div class="new_input-gp">
																<input type="radio" name="new_fontfamily" value="Lobster" id="new_fontfamily2" '; if ( isset ( $new_radio_fontfamily ) ) checked( esc_attr( $new_radio_fontfamily ), "Lobster" ); echo'>
																<label class="new_fontfamily2" for="new_fontfamily2">Happy New Year</label>
															</div>
															<div class="new_input-gp">
																<input type="radio" name="new_fontfamily" value="Dancing_Scrip" id="new_fontfamily3"'; if ( isset ( $new_radio_fontfamily ) ) checked( esc_attr( $new_radio_fontfamily ), "Dancing_Scrip" ); echo'>
														...
														<div class="part-wrap">
															...
															<div class="new_input-gp">
																<input type="radio" checked name="new_color" value="new_color1" id="new_color1"'; if ( isset ( $new_radio_color ) ) checked( esc_attr( $new_radio_color ), "new_color1" ); echo'>
																<label class="new_color1" for="new_color1">Happy New Year</label>
															</div>						 
															<div class="new_input-gp">
																<input type="radio" name="new_color" value="new_color2" id="new_color2"'; if ( isset ( $new_radio_color ) ) checked( esc_attr( $new_radio_color ), "new_color2" ); echo'>
																<label class="new_color2" for="new_color2">Happy New Year</label>
															</div>
															<div class="new_input-gp">
																<input type="radio" name="new_color" value="new_color3" id="new_color3"';if ( isset ( $new_radio_color ) ) checked( esc_attr( $new_radio_color ), "new_color3" ); echo'>
											...
											<div id="new_msg" class="message-setting ';if($new_radio_greetings=='new_message'){ echo esc_attr( "show-option" );	} echo'">
												<div class="tc_bg">
													<div class="tc_head">
														<h3>message</h3>
														<div class="clr"></div>
													...
													<div class="tc_body">					
														<div class="part-wrap">
															<textarea name="new_msg" maxlength="85" rows="4" cols="50" id="new_text_tc" placeholder="Enter message...">'. esc_attr( $new_message_res ).'</textarea>
														</div>
													</div>
												</div>
											</div>
											<div id="new_coupon" class="coupon-code hide-option ';if($new_radio_greetings=='new_coupon'){ echo esc_attr( "show-option" );	} echo'">				
												<div class="tc_bg">	
													<div class="tc_head">
														<h3>COUPON CODE</h3>
														<div class="clr"></div>
													...
													<div class="tc_body">
														<div class="code-input">
															<input type="text" maxlength="10" value="'. esc_attr( $new_coupon_code ) .'"name="new_code" placeholder="Enter code">
														</div>	
													</div>
												</div>
											</div>
											<div id="new_product" class="product-setting hide-option '.esc_attr( @$new_product_show ).'">
												<div class="tc_bg">
													<div class="tc_head">	
														<h3>Product</h3>
														<div class="clr"></div>
													...
													<div class="tc_body">				
														...
														<div class="part-wrap" style="margin-top:0"><h3>Layout</h3>
															<div class="layout-wrap">
																<div>								
																	<input type="radio" name="new_layout" checked value="layout1" id="new_layout1"'; if ( isset ( $new_radio_layout ) ) checked( esc_attr( $new_radio_layout ), "layout1" ); echo'>
																	<label for="new_layout1"><img id="new_layout1_img" class="new_role unselect ';if($new_radio_layout=='layout1'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/product-img1.png"></label>
																</div>
																<div>									
																	<input type="radio" name="new_layout" value="layout2" id="new_layout2" '; if ( isset ( $new_radio_layout ) ) checked( esc_attr( $new_radio_layout ), "layout2" ); echo'>
																	<label  for="new_layout2"><img id="new_layout2_img" class="new_role unselect ';if($new_radio_layout=='layout2'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/product-img2.png"></label>
																</div>
																<div>					
																	<input type="radio" name="new_layout" value="layout3" id="new_layout3" '; if ( isset ( $new_radio_layout ) ) checked( esc_attr( $new_radio_layout ), "layout3" ); echo'>
																	<label for="new_layout3"><img id="new_layout3_img" class="new_role unselect ';if($new_radio_layout=='layout3'){ echo esc_attr( "selected" );	} echo'" src="'.$path.'img/product-img3.png"></label>
																</div>
															</div>
														</div>
													</div>
									...
									<div class="active pains_content">
										...
										<div class="col-50">
											<a href="https://technocrackers.com/christmas-greetings/" target="_blank">
												<img src="'.esc_attr( $path ).'/img/premium.png">
											</a>
							...
							<div data-panes class="vertical-tab-content">
								<div class="tab_premium_general sitedecoration active">';
	    							if($lic_obj->is_technocrackers()){ echo'
										...
										<div class="col-50">
											<a href="https://technocrackers.com/christmas-greetings/" target="_blank">
												<img src="'.esc_attr( plugin_dir_url( __FILE__ ) ).'/img/premium.png">
											</a>
											<div class="content_right">
												<p>Buy Activation Key form Here..</p>
												<p><a href="https://technocrackers.com/christmas-greetings/" target="_blank">Buy Now...</a></p>
	...
	<script type="text/javascript">
		jQuery(window).ready(function(e){
			...
			var page_data = '.wp_json_encode($pages_array).';
			var new_page_data = '.wp_json_encode($new_pages_array).';
...
class christmas_greetings_lic_class {
    ...
    public function show_christmas_gift_status(){
    	$pages_selcetion = json_decode(get_option('display_on_pages'), true);
			$new_pages_selcetion = json_decode(get_option('display_on_pages_new'), true);

			if (
					(get_option('christmas_display_on') == 'home' && is_front_page()) ||
					(get_option('christmas_display_on') == 'all') ||
					(is_array($pages_selcetion) && (
							in_array(get_the_ID(), $pages_selcetion) ||
							(is_home() && in_array(get_option('page_for_posts'), $pages_selcetion))
					)) ||
					(empty($pages_selcetion) && get_option('christmas_display_on') == 'pages')
			) {
